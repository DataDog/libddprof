...
impl Form {
    ...
    pub(crate) fn stream(mut self) -> FormBody {
        if self.inner.fields.is_empty() {
            return FormBody {
                inner: hyper::Body::empty(),
            };
        }
        ...
        let start = Box::pin(self.part_stream(name, part))
            as Pin<Box<dyn Stream<Item = FormResult<Bytes>> + Send + Sync>>;
        ...
        FormBody {
            inner: hyper::Body::wrap_stream(stream.chain(last)),
        }
    }
    ...
    pub(crate) fn part_stream<T>(
        ...
        name: T,
        part: Part,
    ) -> impl Stream<Item = FormResult<Bytes>>
    where
        T: Into<Cow<'static, str>>,
    {
        // start with boundary
        ...
        boundary
            .chain(header)
            .chain(part.value.map_err(|e| e.into()))
...
impl Part {
        ...
        T: Into<Cow<'static, str>>,
    {
        let body = match value.into() {
            Cow::Borrowed(slice) => hyper::Body::from(slice).into(),
            Cow::Owned(string) => hyper::Body::from(string).into(),
        };
        Part::new(body, None)
    }
        ...
        T: Into<Cow<'static, [u8]>>,
    {
        let body = match value.into() {
            Cow::Borrowed(slice) => hyper::Body::from(slice).into(),
            Cow::Owned(vec) => hyper::Body::from(vec).into(),
        };
        Part::new(body, None)
    }
    ...
    pub(crate) fn stream<T: Into<FormBody>>(value: T) -> Part {
    ...
    /// length beforehand.
    pub(crate) fn stream_with_length<T: Into<FormBody>>(value: T, length: u64) -> Part {
        Part::new(value.into(), Some(length))
    }

    fn new(value: FormBody, body_length: Option<u64>) -> Part {
        Part {
    ...
    pub fn mime_str(self, mime: &str) -> FormResult<Part> {
        Ok(self.mime(Mime::from_str(mime).map_err(Box::new)?))
    }
...
mod tests {
    ...
    fn stream_to_end() {
        let mut form = Form::new()
            .part(
                "reader1",
                Part::stream(hyper::Body::wrap_stream(stream::once(future::ready::<
                    FormResult<String>,
                >(
                    Ok("part1".to_owned()),
                ))))
    ...
    fn correct_content_length() {
        ...
        let stream_part =
            Part::stream_with_length(hyper::Body::wrap_stream(the_stream), stream_len as u64);
